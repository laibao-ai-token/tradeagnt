...
- [x] 统一“左侧候选序”和“右侧模型排名”的口径说明（避免误读为同一排序）
- [x] 基金页补充 MRank/sRank 含义提示（页内文案 + 一行帮助说明）
- [x] Top5 展示切换为“领域内 Top5（自动驾驶）”并保留全池榜单入口（避免芯片主题干扰）
- [x] 选票信息固定输出“编号+代码+名称+角色（主选/备选/观察）”
...
- [x] 参数支持 JSON 配置化
- [x] 评估结果可复现
- [x] 左右栏排序口径可解释（用户不再混淆候选序与模型排名）
- [x] Top5 明确为自动驾驶领域榜（可与全池榜切换）
...
### 2026-02-26 01:12（SSU4）

- [x] 基金页口径文案落地：新增 cnd / MRank / sRank 三者解释
- [x] 左侧列头统一：候序 + MRank + sRank，减少“左1右22”误读
- [x] 右下 Top5 调整为“双榜单”：领域相关序 Top5 + 模型评分 Top5（全池）
- [x] 选票信息升级为“编号 + 代码 + 名称 + 角色”固定格式
...
- [x] JSON 配置化落地：
  - 配置文件：`config/etf_profiles.json`
  - 权重/候选池可通过配置文件调整
  - 模块 `etf_profiles.py` 优先加载 JSON 配置，回退到代码内默认值
- [x] 验收标准全部达成（12/12）
