#!/usr/bin/env python3
"""Signal context command - unified domain read model."""
...
TOOL_NAME = "tradecat_get_signal_context"
...
def build_payload(symbol: str, timeframe: str, limit: int, *, runner=run_cmd, now=None) -> dict[str, Any]:
    now = now or datetime.now(timezone.utc)
    warnings: list[str] = []

    signals_source = _normalize_source_result(
        runner(SIGNALS_CMD, ["--symbol", symbol, "--timeframe", timeframe, "--limit", str(limit)]),
        source="signals",
    )
    ...
    result = {
        ...
        "ts": now.isoformat(),
        "source": "local",
        "request": {"symbol": symbol, "timeframe": timeframe, "limit": limit},
        "data": {
            "symbol": symbol,
            "timeframe": timeframe,
            "signals": signals_data,
            "current_price": quotes_data,
            "source_status": {
...
def main():
    ...
    result = build_payload(args.symbol, args.timeframe, args.limit)
