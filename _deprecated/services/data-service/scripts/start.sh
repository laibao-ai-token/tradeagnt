...
ensure_service_venv() {
    ...
    if [[ ! -x "$venv_python" ]]; then
        ...
        echo "   先执行: ./scripts/init.sh data-service"
        return 1
    fi
    if declare -f tc_python_is_compatible >/dev/null 2>&1 && ! tc_python_is_compatible "$venv_python"; then
        echo "❌ data-service 虚拟环境 Python 版本过低: $(tc_python_version_string "$venv_python")"
...
case "${1:-status}" in
    ...
    *)
        ...
        echo ""
        echo "说明:"
        echo "  start    - 默认启动守护模式（先拉起 backfill/metrics/ws，再启动内部 daemon-loop）"
        echo "  stop     - 停止守护进程和全部组件"
        echo "  status   - 查看 daemon 与各组件状态"
        echo "  restart  - 重启全部组件"
        echo ""
        echo "内部命令:"
        echo "  daemon-loop - 仅供脚本内部守护使用，不建议手动调用"
        exit 1
        ;;
esac
